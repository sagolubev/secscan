...
func TestScanMissingPreparationIsActionableAndDoesNotPull(t *testing.T) {
	...
	if string(data) != "info --format {{json .SecurityOptions}}\nversion --format {{json .Server}}\n" {
...
func TestIaCMissingPreparation(t *testing.T) {
	...
	if string(data) != strings.Repeat("info --format {{json .SecurityOptions}}\nversion --format {{json .Server}}\n", 3) {
...
func TestCLIRejectsIaCWithoutEvaluatedInputs(t *testing.T) {
	...
	script := `#!/bin/sh
case "$1" in version) printf '{"Os":"linux","Arch":"amd64"}'; exit 0;; info) printf '[]'; exit 0;; pull) exit 0;; image) printf '%s\n' "$SECSCAN_TEST_IMAGE_ID"; exit 0;; esac
for arg in "$@"; do
...
func TestCLIBearerSkippedOnServerArm64(t *testing.T) {
	...
	bin := t.TempDir()
	script := `#!/bin/sh
case "$1" in info) printf '[]'; exit 0;; version) printf '{"Os":"linux","Arch":"arm64"}'; exit 0;; esac
exit 99
`
	if err := os.WriteFile(filepath.Join(bin, "docker"), []byte(script), 0700); err != nil {
		t.Fatal(err)
...
func TestAcceptanceBearerCoverageContractWithFakeRuntime(t *testing.T) {
	...
	script := `#!/bin/sh
case "$1" in
 info) printf '[]'; exit 0;; pull) exit 0;;
