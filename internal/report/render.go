...
{{if .Commit}}<p class="meta">Commit: <code>{{.Commit}}</code></p>{{end}}{{if .ImageDigest}}<p class="meta">Image: <code>{{.ImageDigest}}</code></p>{{end}}<p class="meta">Fingerprint: <code>{{.Fingerprint}}</code></p></article>{{else}}<p>No visible findings. This does not establish that every input was analysed.</p>{{end}}</section>
