...
и содержательными полями Beads. Он проверяет их до и после выполнения команд.
В upstream run metadata такой привязки нет. [Scope](../internal/tracecheck/git.go)
сравнивается с diff от baseline и неигнорируемыми untracked files.
...
Остаются ручные места: связывание нового scenario с тестом, сохранение фазовых
результатов в Beads, проверка предыдущего evidence и решение о закрытии outcome.
CLI не проверяет последовательность baseline → target → final и не доказывает
независимость review. Эти ограничения нужны в definition of done даже при
зелёной CI. Измерения текущего пилота приведены в [отдельной оценке](grace-pilot.md).
...
Следующая полезная автоматизация — убрать ручное обслуживание evidence внутри
существующего verifier. Он мог бы сравнивать сохранённый результат с текущими
хешами кода и требований без повторного запуска сканеров. Результат нужно
связывать с задачей и различать passed, failed, skipped и validation-only.
Это предложение, такой команды сейчас нет. Добавлять ещё один runner или tracker для этого не нужно.
Полезен и отчёт по уже имеющимся trace-ссылкам: какие требования затронуты diff,
какие проверки выполнены и какие результаты устарели. Выбор только части тестов
допустим после проверки полноты этих связей; обязательные широкие gates остаются.
